...
test('section archive action reflows within every supported 200% viewport', async () => {
  ...
  for (const width of releaseWidths) {
    ...
    assert.match(
      active,
      /\.home-today__grid\s*\{[\s\S]*?min-width:0;[\s\S]*?max-width:100%;[\s\S]*?grid-template-columns:minmax\(0,1fr\);/,
      `${width}px misses the single-column today grid`
    );
    assert.match(
      active,
      /\.home-today-card--wide\s*\{[\s\S]*?width:100%;[\s\S]*?max-width:100%;[\s\S]*?min-width:0;[\s\S]*?grid-column:1\/-1;/,
      `${width}px misses the bounded today-card contract`
    );
    assert.match(
      active,
      /\.home-today-card :is\([\s\S]*?header,[\s\S]*?div,[\s\S]*?span,[\s\S]*?p,[\s\S]*?button,[\s\S]*?\)\s*\{[\s\S]*?min-width:0;[\s\S]*?max-width:100%;[\s\S]*?white-space:normal;[\s\S]*?overflow-wrap:anywhere;/,
      `${width}px misses the inner today-card wrap contract`
    );
    assert.match(
      active,
      /\.home-today-actions[\s\S]*?grid-template-columns:minmax\(0,1fr\);/,
      `${width}px misses the single-column today actions`
    );
  }
});

test('r10m stylesheet and service-worker revisions are atomic', async () => {
  const index = await source('index.html');
  const worker = await source('service-worker.js');
  const stylesheet = 'news-app-2-website.css?release=37';
