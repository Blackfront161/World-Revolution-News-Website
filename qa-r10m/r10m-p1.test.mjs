...
test('r10m browser and service-worker revisions are consistent', async () => {
  ...
  assert.match(worker, /news-app-2-website\.css\?release=37/);
...
test('QA worktree contains no accidental nested deployment packages', async () => {
  ...
  assert.equal(names.some(name => /r10m.*\.(?:zip|aab|apk)$/i.test(name)), false);
});
