...
test('r10k browser and service-worker revisions are consistent', async () => {
  ...
  assert.match(worker, /news-app-2-website\.css\?release=33/);
  assert.match(worker, /wrn-web-portal-2026-08-16-r10k-r40/);
...
test('QA worktree contains no accidental nested deployment packages', async () => {
  ...
  assert.equal(names.some(name => /r10k.*\.(?:zip|aab|apk)$/i.test(name)), false);
});
